            ...
            run: function(inVar, event) {
                ...
                if (this.isDataType('text')) {
                    ...
                    data.image_url = function(callback) {
                        ...
                        document.querySelector(".share").innerHTML = "<i class='fa-solid fa-spinner fa-spin'></i>&nbsp;EGIS";

                       

                        //envoie directement vers le site expertsgroupe

                        const fermerFenetre = function (){
                        ...
                        chrome.tabs.query({ url:"https://*.expertsgroupe.fr/vrades/index" }, function(tabs) {
                            console.log("yes");
                            var tab = tabs[0];
                            var tabId = tab.id;
                        
                            chrome.storage.local.get(['id'], function (item){chrome.tabs.sendMessage(tabId, { id: item, base64:imageData }); fermerFenetre();});
                        ...
                        //fin
                            ...
                            //envoie sur le server nodeJS
                            /*
                            chrome.storage.sync.get(["site", "dossier","id"], function(items) {

                             const datas = {
                                site: items.site,
                                dossier : items.dossier,
                                base64: imageData,
                                filename:items.id
                            };
                                var apiKey = "ZpA7NWN6eqbeXdExs1ox5iqSJBcIFZ7v";
                                const options = {method: 'POST',body: JSON.stringify(datas),headers: {'Content-Type': 'application/json'}};
                                
                                fetch('https://vrades.expertsgroupe.fr/data?token='+apiKey, options)
                                  .then(response => {
                                    if (response.ok) {
                                        chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
                                            var currentTabId = tabs[0].id;
                                  ...
                                  .catch(error => {
                                    console.error(error);
                                    alert('Annonce non-envoyée sur EGIS');
                                  });
                    ...
                    if (typeof inVar == 'string' && inVar.slice(0, 4) == 'data') {
                        ...
                        if (inVar.tagName == 'CANVAS' || inVar.tagName == "IMG") {
                            data.image_data = function(callback) {
                                var ans = canvas.toDataURL("image/png");
                                if (callback) callback(ans);
                                return ans;
                            }
                        } else {
